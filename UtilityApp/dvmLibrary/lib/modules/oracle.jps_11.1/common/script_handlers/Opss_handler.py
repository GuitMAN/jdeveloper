...
def addOpssCommandHelp():  
 try:
   ...
   ora_help.addHelpCommand("syncKeyStores", "opss", offline="false", online="true")
...
def opss_getPrincipalProperties(m) :
    identityDomain = m.get("identityDomain")
    if identityDomain is not None:
        from oracle.security.jps.mas.mgmt.jmx.policy import PortableProperties
        from oracle.security.jps.service.policystore.info import PropertyPrincipalEntry
        props = PortableProperties()
        props.put(PropertyPrincipalEntry.IDENTITY_DOMAIN_PROP_NAME, identityDomain)
        return props
    return None
...
def grantEntitlementImpl(m, on) :
    ...
    try :
        pPl = None
        princType = opss_getPrincipalType(m.get("principalClass"))
        pPl = PortablePrincipal(m.get("principalClass"), m.get("principalName"), princType, opss_getPrincipalProperties(m))
        objectName = ora_mbs.makeObjectName(on)
...
def grantAppRoleImpl(m,on) :
    ...
    try :
        ...
        princType = opss_getPrincipalType(m.get("principalClass"), m.get("forceValidate")) 
	print princType
...
####################################################################
# migrate compatibility check API
####################################################################
def preMigrateCompatibilityCheck(pm,o,skip) :
    msg = opss_resourceBundle.getString(WlstResources.MSG_WLST_MIGRATECHECK_DONE)
    print msg
    if(skip == "true"):
        msg = opss_resourceBundle.getString(WlstResources.MSG_WLST_SKIP_DONE)
        print msg
    ...
    if(skip == "true"):
       ora_mbs.invoke(o, "enableSkip", None, None)
    ora_mbs.invoke(o, "preMigrateCompatibilityCheck", params, sign)
...
def reassociateService(pm,o,s,join,migrate,skip) :    
      
    params = [pm.toCompositeData(None), s] 
    sign = ["javax.management.openmbean.CompositeData","java.lang.String"]
    ora_mbs.invoke(o,"checkServiceSetUp", params, sign)
    if((skip == "true") and (join == "true") and (migrate == "true")):
       ora_mbs.invoke(o, "enableSkip", None, None)
    ...
    if((migrate == "true") and (join == "true")) :
        ora_mbs.invoke(o, "migrateDataWithJoin", None, None)
        msg = opss_resourceBundle.getString(WlstResources.MSG_WLST_DATA_REASS_MIGRATED)
        msg = 'Completed migration in join mode'
        print msg

    #disable skip after the reassociate is done
    if((skip == "true") and (join == "true") and (migrate == "true")):
        ora_mbs.invoke(o, "disableSkip", None, None)
...
def reassociateSecurityStoreImpl(m,on) :
    ...
    if (m.get("migrate") is None) :
        migrate = "false"
        m.remove("migrate")
    else :
        migrate = m.remove("migrate")

    if (m.get("skip") is None) :
        skip = "false"
        m.remove("skip")
    else :
        validateBooleanValue("skip", m.get("skip"))
        skip = m.remove("skip")

    if (m.get("force") is None) :
        force = "false"
        m.remove("force")
    else :
        validateBooleanValue("force", m.get("force"))
        force = m.remove("force")
    ...
    try :
        ...
        if((join == "false") and (force == "true")):
            cleanSecurityStore(pm,o)

        # Do the pre migrate compatibility check for all stores here
	if((migrate == "true") and (join == "true")):
        	preMigrateCompatibilityCheck(pm,o,skip)

        #Reassociate the Policy Store
        s = "POLICY_STORE"               
        msg = opss_resourceBundle.getString(WlstResources.MSG_WLST_POLICY_STORE_REASS_START)
        print msg
        reassociateService(pm,o,s,join,migrate,skip)       
...
# cleanSecurityStore function starts
def cleanSecurityStore(pm,o) :
    msg = opss_resourceBundle.getString(WlstResources.MSG_WLST_CLEAN_SECURITYSTORE_START)
    print msg
    params = [pm.toCompositeData(None)]
    sign = ["javax.management.openmbean.CompositeData"]
    ora_mbs.invoke(o,"cleanSecurityStore", params, sign)
    msg = opss_resourceBundle.getString(WlstResources.MSG_WLST_CLEAN_SECURITYSTORE_END)
    print msg

# cleanSecurityStore function ends
...
def upgradeOpssImpl(m):
    ...
    transformData = m.get("transformData")
    reqArgs = ArrayList()
    ...
    try :
        ...
        print msg
        from java.util import HashMap
        configDetails = HashMap()
        configDetails.put("transformData", transformData)
        Upgrade.upgrade(jpsConfig, jaznData, auditStore, jdbcDriver, url, user, password, Boolean.parseBoolean(upgradeJseStoreType), configDetails)
