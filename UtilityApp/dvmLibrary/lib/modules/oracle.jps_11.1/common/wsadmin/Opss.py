...
def listCodeSourcePermissions(appStripe=None, codeBaseURL=None) :
    opss.listCodeSourcePermissions(appStripe, codeBaseURL)
...
def grantAppRole(appStripe=None, appRoleName=None, principalClass=None, principalName=None, forceValidate="true") :  
    opss.grantAppRole(appStripe, appRoleName, principalClass, principalName, forceValidate)
...
def migrateSecurityStore(type=None, src=None, dst=None, srcApp=None, dstApp=None, srcFolder=None, dstFolder=None, dstLdifFile=None, srcConfigFile=None, configFile=None, overWrite=None, processPrivRole=None, resourceTypeFile=None, migrateIdStoreMapping=None, preserveAppRoleGuid=None, mode=None, reportFile=None) :   
    opss_wlst_or_wsadmin_migration="true";
    from java.lang import System
    System.setProperty("was.offline", "true");
    opss.migrateSecurityStore(type, src, dst, srcApp, dstApp, srcFolder, dstFolder, dstLdifFile, srcConfigFile, configFile, overWrite, processPrivRole, resourceTypeFile, migrateIdStoreMapping, preserveAppRoleGuid, mode, reportFile, opss_wlst_or_wsadmin_migration)
...
def reassociateSecurityStore(domain=None, admin=None, password=None,ldapurl=None,servertype=None,jpsroot=None,join=None, datasourcename=None, jdbcurl=None, dbUser=None, dbPassword=None, jdbcdriver=None, odbcdsn=None, force=None) :    
    opss.reassociateSecurityStore(domain, admin, password,ldapurl,servertype,jpsroot,join,datasourcename,jdbcurl,dbUser,dbPassword,jdbcdriver,odbcdsn,force)
...
def patchPolicyStore(phase=None, patchDeltaFolder=None, productionJpsConfig=None, baselineFile=None, patchFile=None, baselineAppStripe=None, productionAppStripe=None, patchAppStripe=None, silent=None, ignoreEnterpriseMembersOfAppRole=None, reportFile=None, ignoreEnterpriseAppRoleMembershipConflicts=None) :
    return opss.patchPolicyStore(phase, patchDeltaFolder, productionJpsConfig, baselineFile, patchFile, baselineAppStripe, productionAppStripe, patchAppStripe, silent, ignoreEnterpriseMembersOfAppRole, reportFile,   ignoreEnterpriseAppRoleMembershipConflicts)
...
def rollOverEncryptionKey() :
    opss.rollOverEncryptionKey()
...
############################################
# This function migrates the policies to XACML
############################################
...
###########################################################################
# This function restore cretential store/key store encryption key
############################################################################
...
############################################################################
# Bug Fix 18176362, setting was.offline property to true, to address NPE from the JRF Layer in a WAS Env.
def upgradeOpss(jpsConfig=None, jaznData=None, auditStore=None, jdbcDriver=None, url=None, user=None, password=None, upgradeJseStoreType=None, transformData=None):
    from java.lang import System
    System.setProperty("oracle.security.opss.platform.type", "WEBSPHERE");
    System.setProperty("jps.xml.schema.validation.disabled", "true");
    System.setProperty("was.offline", "true");
    opss.upgradeOpss(jpsConfig, jaznData, auditStore, jdbcDriver, url, user, password, upgradeJseStoreType, transformData)

##################################################
# This function rolls over the OPSS encryption key
##################################################
def rollOverEncryptionKey(jpsConfigFile=None):
    opss.rollOverEncryptionKey(jpsConfigFile)

############################################################
# This function updates trust service provider configuration
############################################################
def updateTrustServiceConfig (providerName=None, propsFile=None) :
    opss.updateTrustServiceConfig(providerName,  propsFile)

#######################################################
# This function lists security store type, location and user-name
#######################################################
def listSecurityStoreInfo(domainConfig=None) :
    opss.listSecurityStoreInfo(domainConfig)
