...
optional = frozenset(['-appStripe', '-codeBaseURL', '-principalClass', '-principalName', '-permTarget', '-permActions', '-identityDomain'])
import jpsCmdHelp
...
connect()
import jpsWlstCmd 
jpsWlstCmd.revokePermission(appStripe=appStripe, codeBaseURL=codeBaseURL, principalClass=principalClass, principalName=principalName, permClass=permClass, permTarget=permTarget, permActions=permActions, identityDomain=identityDomain)
