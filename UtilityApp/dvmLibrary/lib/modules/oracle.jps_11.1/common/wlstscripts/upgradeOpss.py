...
optional = frozenset(['-auditStore', '-jdbcDriver', '-url', '-user', '-password', '-upgradeJseStoreType', '-transformData'])
import jpsCmdHelp
...
if 'transformData' in argmap:
        transformData = argmap['transformData']

import jpsWlstCmd
jpsWlstCmd.upgradeOpss(jpsConfig=jpsConfig, jaznData=jaznData, auditStore=auditStore, jdbcDriver=jdbcDriver, url=url, user=user, password=password, upgradeJseStoreType=upgradeJseStoreType, transformData=transformData)
